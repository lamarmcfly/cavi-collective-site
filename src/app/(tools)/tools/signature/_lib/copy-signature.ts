export async function copySignatureToClipboard(
  ...
  // Include both MIME types:
  // - text/plain: raw HTML code for pasting into source/code editors
  // - text/html: rendered version for rich-text paste (bonus, often lossy)
  try {
...
function fallbackCopy(html: string): boolean {
  // Fallback: copy raw HTML as plain text via textarea
